...
pw = tk.StringVar()
e1 = tk.Entry(window, show="*", textvariable = pw)
...
with connection.cursor() as cursor:
    # select all the sales records whose parcel is not arrived and with parcel_id
    sql = "SELECT * FROM sales WHERE arrived = 0 AND (parcel_id IS NOT NULL OR parcel_id != '')"
    cursor.execute(sql)
